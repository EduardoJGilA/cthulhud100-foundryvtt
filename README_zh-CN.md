...
本项目不包含任何官方书籍中的内容，您需要手动输入武器、技能、物品等信息才能进行游戏。我们仅提供了一些基础技能供您参考。
...
- 示例 (Examples)：内含 1920 年代基础人物卡（导入即可创建新角色）、NPC 及怪物模板。
- 技能 (Skills)：提供一整套标准基础技能，可直接应用于角色卡。
- 武器 (Weapons)：包含一把用于测试机制的示例武器。
- 检定请求 (Roll Requests)：内置大量快捷掷骰链接，满足守秘人与玩家的各项检定需求。

## 贡献指南

感谢您有意愿与我们共同完善本系统！我们非常鼓励大家通过提供新功能、修复缺陷 (Bug Fixes) 或优化性能来参与改进本项目。为了让协作过程更加高效顺畅，请您抽出一点时间阅读我们的指南。

如需了解如何提交问题反馈 (Issue)、提交代码更改 (Commit) 以及发起合并请求 (Pull Request)，请仔细参阅我们的 贡献指南 (Contribution Guidelines)。在参与本项目交流与协作时，请务必遵守我们的 行为准则 (Code of Conduct)。
...
本仓库中的所有代码均受 GNU 通用公共许可证 v3.0 (GNU GPLv3) 保护。

对 Foundry VTT 的系统支持受模块开发有限许可协议 (Limited License Agreement for Module Development) 的约束。

本项目使用了来自 Game Icons 的部分素材资产，感谢所有创作者 (Authors)。

本项目使用了由 Manuel Vögele 开发的 socketlib，该库采用 MIT 许可证 授权。

本系统使用了 Chaosium Inc. 和/或 Moon Design Publications LLC 拥有的商标与版权，其使用严格遵循 Chaosium Inc. 粉丝内容政策 (Fan Material Policy)。我们被明确禁止就本内容的使用与访问向您收取任何费用。本系统未经 Chaosium Inc. 的出版、背书或特别批准。如需了解 Chaosium Inc. 产品的更多信息，请访问 www.chaosium.com。

> 特此授权用户在 Foundry VTT 平台上对本作品进行衍生使用。但未授予任何商业使用权。本作品的任何部分均不得在 Foundry VTT 平台之外被复制、分发或使用。Chaosium Inc. 保留所有权利。
> 
> Call of Cthulhu (tm) 属 Chaosium, Inc. 商标。经许可使用。保留所有权利。
>
> 上述版权声明及本许可声明应包含在本软件的所有副本或实质性部分中。
>
> 本软件按“原样 (AS IS)”提供，不附带任何明示或暗示的保证，包括但不限于对适销性、特定用途的适用性以及非侵权性的保证。在任何情况下，作者或版权持有人均不对因本软件、软件的使用或其他交易行为所产生、引起或与之相关的任何索赔、损害赔偿或其他责任负责，无论该责任是基于合同、侵权还是其他诉讼形式。
